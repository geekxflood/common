...
func DebugComponent(component, componentType, msg string, args ...any) {
	allArgs := append([]any{"component", component, "component_type", componentType}, args...)
	Get().Debug(msg, allArgs...)
}
...
func (clf *ComponentLoggerFactory) CreateLogger(componentName, componentType string) *ComponentLogger {
	return NewComponentLogger(componentName, componentType)
}
...
func (clf *ComponentLoggerFactory) CreateLoggerFromComponent(component interface{}) *ComponentLogger {
	if comp, ok := component.(interface {
		Name() string
		Type() string
	}); ok {
		return NewComponentLogger(comp.Name(), comp.Type())
	}

	return NewComponentLogger("unknown", "unknown")
}

var globalLoggerFactory = NewComponentLoggerFactory()
...
func ForComponent(componentName, componentType string) *ComponentLogger {
	return globalLoggerFactory.CreateLogger(componentName, componentType)
}
...
func ForComponentInterface(component interface{}) *ComponentLogger {
	return globalLoggerFactory.CreateLoggerFromComponent(component)
}
